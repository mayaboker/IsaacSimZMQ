...
PACKAGE_ROOT="IsaacSimZMQ"
PACK_DIR="${TEMP_DIR}/${PACKAGE_ROOT}"
mkdir -p "${PACK_DIR}"
...
# Create run script (generated in packaged output, not in this source repo)
...
#
# This script is generated by docker/pack_standalone.sh and lives in the extracted
# standalone package directory (IsaacSimZMQ/run.sh). It is not part of the source repo.
#
# Notes:
# - First argument is optional Isaac Sim path if it points to a valid install.
# - Defaults to full msgpack stream. Set ISAAC_ZMQ_SIMPLE_STREAM=1 before running
#   if you want PUB/SUB simple stream mode.

set -e

DEFAULT_ISAAC_SIM_PATH="/home/$USER/.local/share/ov/pkg/isaac-sim-5.0.0"
ISAAC_SIM_PATH="$DEFAULT_ISAAC_SIM_PATH"

# Accept optional first positional arg as Isaac Sim path
if [ $# -gt 0 ] && [ -f "$1/isaac-sim.sh" ]; then
    ISAAC_SIM_PATH="$1"
    shift
fi

LAUNCHER="${ISAAC_SIM_PATH}/isaac-sim.sh"
if [ ! -f "$LAUNCHER" ]; then
    echo "ERROR: Isaac Sim launcher not found at: $LAUNCHER"
    echo "Usage: ./run.sh [/path/to/isaacsim] --gui --usd /path/to/scene.usd --camera /World/Camera"
    exit 1
fi
...
python3 "${SCRIPT_DIR}/tools/run_zmq_msgpack.py" --launcher "$LAUNCHER" "$@"
EOF
...
cd "${TEMP_DIR}"
tar -czf "${PROJECT_DIR}/${OUTPUT_FILE}" "${PACKAGE_ROOT}"
...
echo "  3. cd ${PACKAGE_ROOT}"
echo "  4. ./install.sh /path/to/isaacsim"
echo ""
echo "Note:"
echo "  - run.sh is generated inside the extracted package (${PACKAGE_ROOT}/run.sh)."
echo "  - It is not a script in the source repository."
echo ""
